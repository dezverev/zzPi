...
`deploy.sh` validates `pi-plugs.catalog.jsonc`, packages these files into `/srv/www/pi-plugs/`, and the public `install-pi-plugs.*` scripts install the selected dependency closure into a target repo's `./.pi/extensions/` directory. Pi then auto-discovers those project-local extensions when started from that target repo.
...
- `extensions/00-zz-subagent-runtime.ts` — internal no-op marker plug that installs the shared child-Pi subagent helper libraries and endpoint config so repo-local custom subagents can import `./lib/child-pi-agent.ts` and `./lib/child-agent-model-options.ts`.
...
- `extensions/import-docs.ts` — adds `/import-docs <name>` to clone/prune configured upstream docs and source references into repo-relative `reference/` paths. Supported imports and prune rules are documented in `extensions/import-docs.config.jsonc`. Use `/import-docs list` to see supported imports. Imported references are off by default for chat context; use `/import-docs scope`, `/import-docs enable <name>`, and `/import-docs disable <name>` to opt reference projects in/out per session.
...
- `extensions/localagent.ts` — adds `/localagent <prompt>` and a `localagent` tool that spawn child Pi processes with the configured local LM Studio model, run general delegated tasks to completion, and inject reports back into the main chat/context. Settings are documented in `extensions/localagent.config.jsonc`.
...
- `extensions/refagent.ts` — adds `/refagent <prompt>` and a `refagent` tool for targeted lookup questions about enabled `/import-docs` references under `reference/`. The child inherits the parent reference scope and should be used instead of reading/searching large reference trees in the main context. Settings are documented in `extensions/refagent.config.jsonc`.
...
- `extensions/pr-review.ts` — adds `/pr-review` and a `prreview` tool that run PR-oriented git status/log/diff review in a child Pi process, returning a concise readiness report without loading raw patches into the main context. Settings are documented in `extensions/pr-review.config.jsonc`.
...
- `extensions/context-files.config.jsonc` — file-context pane limits, token estimate ratio, and auto-show behavior.
...
- `extensions/import-docs.config.jsonc` — supported reference imports, targets, overlays, prune rules, and metadata path.
...
- `extensions/localagent.config.jsonc` — local-agent child Pi model, optional endpoint override, tool allowlist, timeouts, and report limits.
...
- `extensions/refagent.config.jsonc` — reference-agent child Pi model, optional endpoint override, read-only tool allowlist, inherited reference scope behavior, timeouts, and report limits.
...
- `extensions/pr-review.config.jsonc` — PR-review child Pi model, shared remote-local endpoint selection, read-only git tool allowlist, timeouts, and report limits.
