...
- `extensions/implementationsubagent.config.jsonc` — implementation child model options, GPT-5.6 Sol max default, mutating code/test tools, and nested readsubagent/debuggersubagent access; vetting remains a main-agent responsibility.

## Implementation

The old standalone `implementation-loop`, `impplanner`, and `implementeragent` products remain retired. The distinct `implementationsubagent` replacement keeps decomposition and orchestration in the main agent, accepts one context-rich Markdown implementation document plus one bounded piece per call, and maintains a readable ledger under `docs/artifacts/implementationdocs/ledgers`. Use `workflowmode` when the full internal `wf-*` planning/implementation/review pipeline is desired. The standalone design stack remains available through `design-loop`, `brainstormer`, and `designplanner`.
